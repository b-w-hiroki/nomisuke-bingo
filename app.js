...
class NomisukeBingo {
  ...
  confirmResult() {
    ...
    // キャプチャエリアに内容をコピー
    document.getElementById('resultCaptureTitle').textContent = '呑み助ビンゴ';
  ...
  downloadResultImage() {
    ...
    html2canvas(area, {
      ...
      link.download = 'nomisuke-bingo-' + new Date().toISOString().slice(0, 10) + '.png';
  ...
  resetAndGoToTop() {
    try {
      localStorage.removeItem('nomisukeBingo');
    } catch (e) {}
      ...
      navigator.clipboard.writeText(this.roomCode)
        .then(() => {
          ...
          if (roomCodeStat) {
            ...
            roomCodeStat.style.background = '#d4a574';
  ...
  generateRoomCode() {
    const words = [
      'えだまめ', 'やきとり', 'にほんしゅ', 'からあげ', 'ほっぴー',
      'れもさわ', 'はいぼー', 'いざかや', 'のみほう', 'しめらー',
      'だいごろ', 'はしご', 'のれん', 'かんぱい', 'おつまみ',
      'びーる', 'しょうちゅう', 'うめしゅ', 'ちゅーはい', 'さわー',
      'さしみ', 'おでん', 'やきにく', 'ぎょうざ', 'てんぷら',
      'ちょうちん', 'かうんたー', 'おしぼり', 'ざしき', 'おかみ',
      'ほろよい', 'いっぱい', 'にくじゃが', 'もつに', 'あげもの',
      'くしかつ', 'たこわさ', 'つけもの', 'こはだ', 'やきそば'
    ];
  ...
  saveToStorage() {
    try {
      ...
      localStorage.setItem('nomisukeBingo', JSON.stringify(data));
      console.log('💾 保存しました');
    } catch (error) {
  ...
  loadFromStorage() {
    try {
      const json = localStorage.getItem('nomisukeBingo');
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('🚀 呑み助ビンゴを起動します');
  const game = new NomisukeBingo();
  game.init();
